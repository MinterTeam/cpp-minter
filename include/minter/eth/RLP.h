...
public:
    ...
    /// Construct a node of value given in the bytes.
    explicit RLP(bytes const& _d, Strictness _s = VeryStrict)
        : RLP(&_d, _s) {
    }
    ...
    explicit operator eth::bytes_data() const {
        return toolbox::data::bytes_data(toBytes());
    }
...
public:
    ...
    RLPStream& append(const toolbox::data::bytes_data& data) {
...
MINTER_TX_API std::ostream& operator<<(std::ostream& _out, eth::RLP const& _d);
