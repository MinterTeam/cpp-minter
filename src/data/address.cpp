...
minter::data::address::address(const uint8_t* data, size_t len)
...
minter::data::address::address(const char* hexString) {
    ...
    std::string address = toolbox::strings::substr_replace_all_ret(
        std::vector<std::string>{"Mx", "mx"},
        std::vector<std::string>{"", ""},
        std::string(hexString));

    m_data = toolbox::data::hex_to_bytes(address);
}
...
minter::data::address::address(const minter::privkey_t& priv_key)
    : address(priv_key.get_public_key()) {
}
